...
public class Matrix {
     ...
     * @param result the array that holds the output matrix
    ...
    public static void multiplyMM(float[] result, float[] rhs, float[] lhs) {
	    ...
	    for (int i = 0; i < 16; i++){
            a = (i >> 2) << 2;
            b = i & 0x00000003;
	        result[i] = (lhs[a] * rhs[b]) + (lhs[a+1] * rhs[b+4]) + (lhs[a+2] * rhs[b+8]) + (lhs[a+3] * rhs[b+12]);
	    }
    }
    ...
    public static void translateM(float[] m, float x, float y) {
        m[12] += m[0] * x;
        m[13] += m[5] * y;
    }
